...
class Trainer:
    ...
    def train(self, data_loader, epochs=10, save_training_gif=None):
        ...
        save_training_gif : None or tuple (path, Visualizer instance)
        ...
        for epoch in range(epochs):
            mean_epoch_loss, mean_epoch_kl_loss = self._train_epoch(data_loader)
            training_kl_loss.append(mean_epoch_kl_loss)
            training_cost.append(mean_epoch_loss)
            print('Epoch: {} Average loss: {:.2f} kl loss: {:.2f} '.format(epoch + 1, mean_epoch_loss, mean_epoch_kl_loss))
        ...
        if save_training_gif is not None:
            imageio.mimsave(save_training_gif[0], training_progress_images,
                            fps=24)
        return training_cost, training_kl_loss
    ...
    def _train_epoch(self, data_loader):
        ...
        for batch_idx, (data, label) in enumerate(data_loader):
            iter_loss, kl_loss = self._train_iteration(data)
        ...
        return epoch_loss / len(data_loader.dataset), epoch_kl_loss / len(data_loader.dataset)
    ...
    def _train_iteration(self, data):
        ...
        loss, kl_loss = self._loss_function(data, recon_batch, latent_dist)
        loss.backward()
        self.optimizer.step()

        train_loss = loss.item()
        return train_loss, kl_loss.item()
    ...
    def _loss_function(self, data, recon_data, latent_dist):
        ...
        if self.model.training and self.num_steps % self.record_loss_every == 1:
            ...
            self.losses['mean_loss'].append(total_loss.item() / self.model.num_pixels)

        # To avoid large losses normalise by number of pixels
        return total_loss / self.model.num_pixels, (cont_capacity_loss + disc_capacity_loss)/self.model.num_pixels
