import numpy as np
import torch
from viz.latentTraversals import LatentTraverser
...
class Visualizer():
    def __init__(self, model):
        ...
        self.save_images = True  # If false, each method returns a tensor instead of saving image.
...
def reorder_img(orig_img, reorder, by_row=True, img_size=(3, 32, 32), padding=2):
    ...
    for new_idx, old_idx in enumerate(reorder):
        if by_row:
            ...
            reordered_img[:, start_pix_new:start_pix_new + height, :] = orig_img[:,
                                                                        start_pix_old:start_pix_old + height, :]
        else:
            start_pix_new = new_idx * (padding + width) + padding
            start_pix_old = old_idx * (padding + width) + padding
            reordered_img[:, :, start_pix_new:start_pix_new + width] = orig_img[:, :,
                                                                       start_pix_old:start_pix_old + width]
